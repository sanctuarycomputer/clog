...
fn observe_bumps_rev_and_renders() {
    ...
    let ack = c
        .observe(
            vec![claim("a", "first thing", 500_000)],
            ObserveOpts::default(),
        )
        .unwrap();
    assert_eq!(ack.rev, 1);
...
fn inv5_duplicate_observe_is_invisible() {
    ...
    c.advance(1_000_000).unwrap();
    let a1 = c
        .observe(vec![claim("a", "x", 500_000)], ObserveOpts::default())
        .unwrap();
...
fn inv4_upsert_supersedes() {
    ...
    c.advance(1_000_000).unwrap();
    c.observe(
        vec![claim("a", "old body", 500_000)],
        ObserveOpts::default(),
    )
    .unwrap();
    c.observe(
        vec![claim("a", "new body", 600_000)],
        ObserveOpts::default(),
    )
    .unwrap();
    let s = c.situation(None, None).unwrap();
...
fn inv3_retraction_heals_text() {
    ...
    c.observe(vec![claim("a", "temp", 500_000)], ObserveOpts::default())
        .unwrap();
    c.retract("a").unwrap();
...
fn inv9_rev_survives_reopen() {
    let dir = tempfile::tempdir().unwrap();
    {
        let c = Clog::open(cfg(dir.path())).unwrap();
        c.advance(1_000_000).unwrap();
        c.observe(vec![claim("a", "x", 500_000)], ObserveOpts::default())
            .unwrap();
        c.observe(vec![claim("b", "y", 500_000)], ObserveOpts::default())
            .unwrap();
    } // drop -> clean shutdown
    let c = Clog::open(cfg(dir.path())).unwrap();
    c.advance(2_000_000).unwrap();
    let ack = c
        .observe(vec![claim("c", "z", 500_000)], ObserveOpts::default())
        .unwrap();
    assert_eq!(ack.rev, 3);
    let s = c.situation(None, None).unwrap();
...
fn reopen_reproduces_situation_text_byte_identically() {
    ...
    let before = {
        let c = Clog::open(cfg(dir.path())).unwrap();
        c.advance(1_000_000).unwrap();
        c.observe(vec![claim("a", "first", 500_000)], ObserveOpts::default())
            .unwrap();
        ...
        c.observe(vec![claim("b", "second", 900_000)], ObserveOpts::default())
            .unwrap();
...
fn reserved_namespace_rejected_and_batch_atomic() {
    ...
    let r = c.observe(
        vec![
            claim("ok", "fine", 500_000),
            claim("clog:sneaky", "no", 500_000),
        ],
        ObserveOpts::default(),
    );
    assert!(r.is_err());
...
fn rules_tier_classifies_at_commit() {
    ...
    c.advance(1_000_000).unwrap();
    c.observe(
        vec![claim("inv", "invoice 1042 is overdue", 500_000)],
        ObserveOpts::default(),
    )
    .unwrap();
    let s = c.situation(None, None).unwrap();
    assert!(
        s.text.contains("- RISK invoice 1042"),
        "open loops slot should show it:\n{}",
        s.text
    );
}
...
fn only_the_surviving_version_of_a_key_is_classified() {
    ...
    c.observe(
        vec![
            claim("inv", "invoice 1042 is overdue", 500_000),
            claim("inv", "invoice 1042 is paid", 600_000),
        ],
        ObserveOpts::default(),
    )
    .unwrap();
    ...
    assert!(
        !s.text.contains("RISK"),
        "surviving claim must not inherit the superseded version's kind:\n{}",
        s.text
    );
...
fn custom_template_and_errors() {
    ...
    c.advance(1_000_000).unwrap();
    c.observe(
        vec![claim("a", "hello world", 500_000)],
        ObserveOpts::default(),
    )
    .unwrap();
    let s = c
        .situation(None, Some("URGENT ONLY\n%{urgent limit=1}"))
        .unwrap();
    assert!(s.text.starts_with("URGENT ONLY\n1."), "{}", s.text);
    assert!(matches!(
        c.situation(None, Some("%{bogus}")),
        Err(ClogError::TemplateError(_))
    ));
    assert!(matches!(
        c.situation(Some("nope"), None),
        Err(ClogError::UnknownScope)
    ));
}
...
fn hidden_item_change_keeps_the_document_but_refreshes_its_inputs() {
    ...
    let batch: Vec<Claim> = (0..10)
        .map(|i| claim(&format!("k{i:02}"), &format!("body {i:02}"), 500_000))
        .collect();
    ...
    let settled = c
        .observe(
            vec![claim("k09", "rewritten tail", 500_000)],
            ObserveOpts::default(),
        )
        .unwrap();
    ...
    let ack = c
        .observe(
            vec![claim("k08", "second rewrite", 500_000)],
            ObserveOpts::default(),
        )
        .unwrap();
    ...
    assert_eq!(
        ...
        "a hidden item's body never reaches the document"
    );
    assert_eq!(
        after.rev, before.rev,
        "unchanged text must keep its rev (rev skew is the signal)"
    );
    assert_eq!(
        after.as_of, before.as_of,
        "and its as_of: nothing material changed"
    );
    assert!(
        after.rev < ack.rev,
        "the scope's rev now lags the global rev, as it should"
    );
    ...
    let wide = c
        .situation(None, Some("%{header}\n%{urgent limit=10}"))
        .unwrap();
    assert!(wide.text.contains("rewritten tail"), "{}", wide.text);
    assert!(wide.text.contains("second rewrite"), "{}", wide.text);
    assert!(
        !wide.text.contains("body 08") && !wide.text.contains("body 09"),
        "{}",
        wide.text
    );
    assert!(
        wide.text
            .starts_with(&format!("default · rev {}", after.rev)),
        "{}",
        wide.text
    );
}
...
fn select_live_with_filters() {
    ...
    let f = Filter {
        observer: Some(ObserverId::from("twist")),
        ..Filter::default()
    };
    ...
    assert!(matches!(
        c.select(View::Live, f),
        Err(ClogError::InvalidFilter { .. })
    ));
...
fn inv6_revoke_observer_one_batch() {
    ...
    let mut a = claim("a", "alpha", 100_000);
    a.observer = ObserverId::from("gmail");
    let mut b = claim("b", "beta", 100_000);
    b.observer = ObserverId::from("gmail");
    c.observe(vec![a, b], ObserveOpts::default()).unwrap();
...
fn p7_shape_merge_round_trip_via_api() {
    ...
    let f = Filter {
        entities: Some(vec![b.clone()]),
        ..Filter::default()
    };
    assert_eq!(c.select(View::Live, f).unwrap().len(), 1);
    // cycle rejected
    assert!(matches!(
        c.merge_entities(&b, &a),
        Err(ClogError::AliasCycle)
    ));
    ...
    assert!(
        c.select(View::Live, Filter::default())
            .unwrap()
            .iter()
            .all(|r| !r.claim.claim_key.starts_with("clog:"))
    );
...
fn select_filters_and_compose_and_limit_is_bounded() {
    ...
    assert!(
        loops
            .iter()
            .all(|r| r.kind.as_ref().is_some_and(|k| k.kind == "risk"))
    );
...
fn select_believed_flag_and_entity_state_rows() {
    ...
    c.observe(vec![loser, winner, loose], ObserveOpts::default())
        .unwrap();
...
fn revoke_of_an_unknown_observer_commits_nothing() {
    ...
    let ack = c
        ...
        .unwrap();
    assert_eq!(
        c.revoke_observer(&ObserverId::from("nobody")).unwrap().rev,
        ack.rev
    );
    ...
    assert_eq!(
        c.revoke_observer(&ObserverId::from("test")).unwrap().rev,
        done.rev
    );
}
...
fn merge_self_is_a_cycle_and_re_merging_is_a_no_op() {
    ...
    assert!(matches!(
        c.merge_entities(&a, &a),
        Err(ClogError::AliasCycle)
    ));
    let first = c.merge_entities(&a, &b).unwrap();
    assert_eq!(
        c.merge_entities(&a, &b).unwrap().rev,
        first.rev,
        "identical merge must not commit"
    );
    ...
    assert!(matches!(
        c.retract("clog:merge:p:a->p:a"),
        Err(ClogError::UnknownClaim)
    ));
    ...
    assert!(matches!(
        c.merge_entities(&bad, &b),
        Err(ClogError::InvalidClaim { .. })
    ));
}
...
fn entity_state_select_is_uncapped_while_the_rendered_slot_still_summarizes() {
    ...
    assert_eq!(
        rows.iter()
            .map(|r| r.claim.subject_key.as_deref().unwrap_or(""))
            .collect::<Vec<_>>(),
    ...
    let line = text
        .lines()
        .find(|l| l.starts_with("Halcyon: "))
        .expect("entities slot");
    ...
    assert!(
        !line.contains("subject 00") && !line.contains("subject 01"),
        "{line}"
    );
...
fn entity_state_select_reports_a_multi_entity_claim_once() {
    ...
    assert_eq!(
        c.select(View::EntityState, Filter::default())
            .unwrap()
            .len(),
        1
    );
    let f = Filter {
        entities: Some(vec![x, y]),
        ..Filter::default()
    };
    assert_eq!(c.select(View::EntityState, f).unwrap().len(), 1);
}
