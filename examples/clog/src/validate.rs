...
use crate::types::{Claim, ClogError, Focus, KindTaxonomy};
...
pub(crate) fn validate_claim(
    ...
    if has_control(&c.claim_key) {
        return Err(invalid(
            index,
            "claim_key must not contain control characters",
        ));
    }
    ...
    if let Some(subject_key) = &c.subject_key {
        ...
        if has_control(subject_key) {
            return Err(invalid(
                index,
                "subject_key must not contain control characters",
            ));
        }
    }
    ...
    if has_control(&c.source_ref) {
        return Err(invalid(
            index,
            "source_ref must not contain control characters",
        ));
    }
    ...
    for e in &c.entities {
        ...
        if has_control(&e.etype) {
            return Err(invalid(
                index,
                "entity etype must not contain control characters",
            ));
        }
...
mod tests {
    ...
    fn u_val_1_claim_rules() {
        ...
        let cases: Vec<(Box<dyn Fn(&mut Claim)>, bool, &str)> = vec![
            ...
            (
                Box::new(|c| c.claim_key = "x".repeat(257)),
                false,
                "claim_key",
            ),
            (
                Box::new(|c| c.claim_key = "clog:evil".into()),
                false,
                "reserved",
            ),
            (
                Box::new(|c| c.claim_key = "has\u{0007}bell".into()),
                false,
                "control",
            ),
            (
            ...
            (
                Box::new(|c| c.subject_key = Some("has\u{0007}bell".into())),
                false,
                "control",
            ),
            (Box::new(|c| c.source_ref = "".into()), false, "source_ref"),
            (
                Box::new(|c| c.source_ref = "x".repeat(1025)),
                false,
                "source_ref",
            ),
            (
                Box::new(|c| c.source_ref = "has\u{0007}bell".into()),
                false,
                "control",
            ),
            (
                Box::new(|c| c.observer = ObserverId(String::new())),
                false,
                "observer",
            ),
            (
                Box::new(|c| c.observer = ObserverId("x".repeat(129))),
                false,
                "observer",
            ),
            (Box::new(|c| c.body = "   ".into()), false, "body"),
            (
                Box::new(|c| c.body = "x".repeat(16 * 1024 + 1)),
                false,
                "body",
            ),
            (
            ...
            if !ok {
                match r.unwrap_err() {
                    ClogError::InvalidClaim { index, reason } => {
                        assert_eq!(index, 7);
                        assert!(
                            reason.to_lowercase().contains(why),
                            "case {i}: {reason} !~ {why}"
                        );
                    }
    ...
    fn scoring_clamp_only_beyond_24h() {
        ...
        assert_eq!(
            scoring_clamp(1_000_000 + 86_400_000, 1_000_000),
